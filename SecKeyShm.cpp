...
SecKeyShm::SecKeyShm(int key, int maxNode)
	: BaseShm(key, maxNode * sizeof(SecKeyNodeInfo))
	, m_maxNode(maxNode)
{
}

SecKeyShm::SecKeyShm(string pathName, int maxNode)
	: BaseShm(pathName, maxNode * sizeof(SecKeyNodeInfo))
	, m_maxNode(maxNode)
{
}
...
int SecKeyShm::shmWrite(SecKeyNodeInfo* pNodeInfo)
{
	...
	int i = 0;
	SecKeyNodeInfo tmpNodeInfo; //空结点
	for (i = 0; i < m_maxNode; i++)
	{
		pNode = pAddr + i;
		if (memcmp(&tmpNodeInfo, pNode, sizeof(SecKeyNodeInfo)) == 0)
		{
			ret = 0;
			memcpy(pNode, pNodeInfo, sizeof(SecKeyNodeInfo));
...
SecKeyNodeInfo SecKeyShm::shmRead(string clientID, string serverID)
{
	int ret = 0;
	// 关联共享内存
	SecKeyNodeInfo*pAddr = NULL;
	pAddr = static_cast<SecKeyNodeInfo*>(mapShm());
	if (pAddr == NULL)
	{
		cout << "共享内存关联失败..." << endl;
